...
#define TEST_NAME "api_knet_link_insert_acl"
...
static void test(void)
{
	...
	if ((!knet_link_insert_acl(NULL, 1, 0, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT)) || (errno != EINVAL)) {
		log_test(logfd, "knet_link_insert_acl accepted invalid knet_h or returned incorrect error: %s", strerror(errno));
		TEST_EXIT(FAIL);
	}

	knet_h1 = _ts_knet_handle_start(logfd, KNET_LOG_DEBUG, knet_h);

	log_test(logfd, "Test knet_link_insert_acl with unconfigured host");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with unconfigured link");
	FAIL_ON_ERR(knet_host_add(knet_h1, 1));
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with invalid link");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, KNET_MAX_LINK, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with invalid ss1");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, NULL, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with invalid ss2");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, &lo, NULL, CHECK_TYPE_RANGE, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with non matching families");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, &lo, &lo6, CHECK_TYPE_RANGE, CHECK_ACCEPT), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with wrong check_type");
	...
	log_test(logfd, "Test knet_link_insert_acl with wrong acceptreject");
	FAIL_ON_SUCCESS(knet_link_insert_acl(knet_h1, 1, 0, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT + CHECK_REJECT + 1), EINVAL);

	log_test(logfd, "Test knet_link_insert_acl with point to point link");
	...
	log_test(logfd, "Test knet_link_insert_acl with dynamic link");
...
{
	printf("[TEST] %s: Test knet link insert acl\n", TEST_NAME);
