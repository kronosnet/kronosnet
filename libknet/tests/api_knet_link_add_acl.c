...
#define TEST_NAME "api_knet_link_add_acl"
...
static void test(void)
{
	...
	log_test(logfd, "Test knet_link_add_acl with wrong acceptreject");
	FAIL_ON_SUCCESS(knet_link_add_acl(knet_h1, 1, 0, &lo, &lo, CHECK_TYPE_ADDRESS, CHECK_ACCEPT + CHECK_REJECT + 1), EINVAL);

	log_test(logfd, "Test knet_link_add_acl with point to point link");
	...
	log_test(logfd, "Test knet_link_add_acl with dynamic link");
...
{
	printf("[TEST] %s: Test knet link add acl\n", TEST_NAME);
