...
#ifdef KNET_SOLARIS
/*
 * Yes, this is INSANE, but nothing else I can find
 * seems to return the right number.
 */
	snprintf(cmd, sizeof(cmd), "ifconfig %s|head -1|sed -n 's/.*mtu \\([0-9]\\+\\).*/\\1/p'", nozzle->name);
	err = execute_bin_sh_command(cmd, &error_string);
	if (err == 0 && error_string) {
		err = strtol(error_string, 0, 10);
		free(error_string);
	}
	goto out_clean;
#else
...
int solaris_setup_tap(nozzle_t nozzle, char *devname, int namelen)
{
	...
	if (unit_nr < 0) {
		perror("Failed to create new tap");
		close(tap_fd);
		return -EIO;
	}

	if (ioctl(tap_fd, I_SRDOPT, RMSGD) < 0) {
		perror("Failed to put tap file descriptor into message-discard mode");
		close(tap_fd);
		return -EIO;
	}
	...
	if (strlen(devname) == 0) {
		...
		if (sscanf(devname, "tap%d", &unit_nr) == 1) {
			...
			if ((newppa = ioctl(tap_fd, I_STR, &strioc_ppa)) < 0) {
				perror("Unable to set PPA on device");
				return -errno;
			}
		} else {
			return -EIO;
